...
export async function createTodayPollImg(pollId) {
  ...
  // 2) 타겟 URL 접속
  const targetUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/polls/open?pollId=${pollId}&type=poll`;
  ...
  // 3) 특정 DOM 요소 스크린샷
  const element = await page.$(".today-poll-wrapper");
  if (!element) {
    throw new Error("Element .today-poll-wrapper not found");
  }
